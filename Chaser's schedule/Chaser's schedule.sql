...
-- After sprinting, base speed s will permanently reduced by 1, and for next one unit of time 
...
-- you are given a table 'speedandtime' with columns 's' (speed) and 't' (time)
-- return a table with these columns and your result in a column named 'res'.
SELECT
  s,
  t,
  s * (t - 2 * n) + 3 * n * (2 * s - n + 1) / 2 AS res
FROM
  speedandtime,
  LEAST ((t + 1) / 2, (s - 2 * s / 3)) n;