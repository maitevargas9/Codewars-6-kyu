...
-- Every year he takes photos of farmers handshaking. Johnny knows that no two farmers handshake more than once. He also knows that some of 
...
SELECT
  ...
  CASE 
    WHEN n = 0 THEN 0
    ELSE CEIL((1 + SQRT(1 + 8 * n)) / 2.0)::INT
  END AS res
FROM participants;